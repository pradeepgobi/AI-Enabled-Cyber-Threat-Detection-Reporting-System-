...
# -------------------------------------------------------------------
# SECRETS / API KEYS (loaded from environment variables)
# -------------------------------------------------------------------
...
def submit_complaint():
    ...
    try:
        ...
        # Build the plaintext user_info from the form, but DO NOT store it directly.
        user_info_plain = {
        ...
        # --- RSA: ENCRYPT SENSITIVE USER INFO BEFORE STORING ----------------
        user_info_encrypted = {}
        try:
            for k, v in user_info_plain.items():
                # Only encrypt non-empty values; store None for missing values
                if v is not None and v != "":
                    user_info_encrypted[k] = rsa_encrypt_text(str(v))
                else:
                    user_info_encrypted[k] = None
        except Exception as e:
            print(f"❌ RSA encryption failed for user_info: {e}")
            return jsonify({"success": False, "error": "Encryption failure"}), 500
        # ----------------------------------------------------------------------
        ...
        ai_summary = get_ai_summary(
            {"attack_type": attack_type, "risk_level": risk_level, "scenario": scenario}
        )

        # --- Build sensitive_encrypted map and store ciphertext only ---
        sensitive_encrypted = {}
        try:
            # attack_type
            sensitive_encrypted["attack_type"] = rsa_encrypt_text(attack_type) if attack_type else None
            # analysis_results -> JSON string then encrypted
            ar_json = json.dumps(analysis_results) if analysis_results else None
            sensitive_encrypted["analysis_results"] = rsa_encrypt_text(ar_json) if ar_json else None
            # ai_assistance
            sensitive_encrypted["ai_assistance"] = rsa_encrypt_text(ai_assistance) if ai_assistance else None
            # ai_summary
            sensitive_encrypted["ai_summary"] = rsa_encrypt_text(ai_summary) if ai_summary else None
            # risk_level
            sensitive_encrypted["risk_level"] = rsa_encrypt_text(risk_level) if risk_level else None
        except Exception as e:
            print(f"❌ RSA encryption failed for sensitive fields: {e}")
            return jsonify({"success": False, "error": "Encryption failure"}), 500
        # --------------------------------------------------------------------

        # 4. Prepare Mongo document
        # NOTE:
        #   - user_info is ONLY stored encrypted.
        #   - scenario/additional_info are stored encrypted only.
        #   - attack_type, risk_level, ai_summary are ALSO stored in plaintext
        ...
        complaint_data = {
            ...
            # Plaintext, non-personal fields for dashboards / filters
        ...
        # Return response compatible with OLD backend (code 2)
        return jsonify(
            {
                "success": True,
                "complaint_id": complaint_id,
...
# Works for BOTH /send-otp and /send-user-otp
...
# Works for BOTH /verify-otp and /verify-user-otp
