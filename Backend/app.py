...
class MultiLibraryPhishingDetector:
    ...
    def _load_suspicious_keywords(self):
        return {
            ...
            "financial": [
                "verify account", "confirm identity", "update payment", "billing problem",
    ...
    def analyze_url_structure(self, url):
        ...
        scores = {
            "length_suspicious": 0, "special_chars": 0, "subdomain_count": 0,
            "path_depth": 0, "has_port": 0, "encoded_chars": 0, "suspicious_keywords": 0,
        }
        reasons = []
        ...
        return {"score": total_score, "is_suspicious": total_score > 0.3, "reasons": reasons, "details": scores}

    def analyze_domain_reputation(self, domain):
        results = {
            "age_days": None, "registrar": None, "creation_date": None,
            "expiry_date": None, "dns_records": {}, "is_suspicious": False, "reasons": [],
        }
    ...
    def analyze_ssl_certificate(self, domain):
        results = {
            ...
            "valid_from": None, "valid_until": None, "is_suspicious": False, "reasons": [],
        }
    ...
    def analyze_page_content(self, url):
        results = {
            "title": None, "forms_count": 0, "iframes_count": 0, "external_links": 0,
            "suspicious_forms": False, "hidden_elements": 0, "is_suspicious": False, "reasons": [],
        }
    ...
    def detect_typosquatting(self, domain):
        results = {
            "is_typosquatting": False, "similar_to": None, "technique": None,
        ...
        popular_brands = [
            ...
            "bestbuy", "bankofamerica", "chase", "wells", "citibank", "americanexpress",
            "discover", "bank", "login", "signin", "secure", "account", "verify", "update",
        ]
        ...
        if any(full_domain.endswith(tld) for tld in suspicious_tlds):
            for brand in popular_brands:
                if brand in full_domain:
                    ...
                    results["reasons"].append("Uses suspicious TLD with brand name")
                    break
    ...
    def analyze_ip_address(self, domain):
        results = {
            "ip_address": None, "is_ip_url": False, "is_private": False,
    ...
    def detect_malware_indicators(self, url):
        results = {
            "has_malware_extension": False, "has_suspicious_params": False,
            "file_extension": None, "is_suspicious": False, "reasons": [],
        }
        ...
        dangerous_exts = [
            ...
            ".app", ".deb", ".rpm", ".dmg", ".pkg", ".msi", ".dll", ".sys", ".drv",
            ".bin", ".run", ".apk", ".zip", ".rar", ".7z",
        ]
    ...
    def comprehensive_analysis(self, url):
        ...
        weights = {
            "url_structure": 0.15, "domain_age": 0.20, "ssl": 0.15,
            "content": 0.15, "typosquatting": 0.20, "ip": 0.10, "malware": 0.05,
        }
        ...
        verdict = self._calculate_verdict(
            risk_score, url, domain, domain_analysis, ssl_analysis, typosquatting, malware_analysis,
        )
    ...
    def _calculate_verdict(self, risk_score, url, domain, domain_analysis, ssl_analysis, typosquatting, malware_analysis):
        if malware_analysis["is_suspicious"]:
        ...
        has_suspicious_keyword = any(keyword in domain_lower or keyword in url_lower for keyword in suspicious_patterns)
...
def analyze_phishing_document(file_path, filename):
    try:
        ...
        return {
            ...
            "risk_level": "high"
            if any(
                r.get("final_verdict") in ["phishing", "HIGH_RISK_PHISHING", "MALWARE_DELIVERY_RISK"]
...
def analyze_deepfake_image(file_path):
    ...
    try:
        ...
        return {"prediction": predicted_label, "confidence": confidence, "is_deepfake": predicted_label.lower() != "real"}
    except Exception as e:
...
def determine_risk_level(analysis_results):
    ...
    if "phishing_document" in analysis_results:
        ...
        if any(
            report.get("verdict_detailed") in ["CRITICAL_PHISHING", "CRITICAL_MALWARE_RISK", "HIGH_RISK"]
...
"""
        ...
        is_deepfake = analysis_results.get("deepfake_image", {}).get("is_deepfake") or \
                      analysis_results.get("deepfake_video", {}).get("is_deepfake")
...
def get_evidence_file(complaint_id, unique_filename):
    ...
    file_meta = next((f for f in evidence_files if f.get("unique_filename") == unique_filename), None)

    if not file_meta:
        return jsonify({"error": "File not found for this complaint"}), 404

    rel_path = file_meta.get("file_path") or os.path.join(app.config["UPLOAD_FOLDER"], unique_filename)
...
def quick_apk_analyser():
    ...
    if apk_model is None or apk_features is None:
        return jsonify({"success": False, "error": "APK ML models are not loaded."}), 500
    ...
    try:
        ...
        return jsonify({"success": False, "error": f"APK Analysis Failed: {str(e)}"}), 500
...
def submit_complaint():
    ...
    try:
        ...
        print(f"✅ Complaint {complaint_id} saved to MongoDB ID: {result.inserted_id}")
        ...
        if complaint:
            ...
            return jsonify({
                ...
                "complaint": {
                    ...
                    "additional_info": complaint.get("additionalInfo") or complaint.get("additional_info"),
...
def get_all_complaints():
    if complaints_collection is None:
        return jsonify({"success": False, "error": "Database connection failed."}), 500
    try:
        projection = {"_id": 0, "analysis_results": 0, "ai_assistance": 0, "admin_notes": 0, "evidence_files": 0}
        ...
        for complaint in complaints_cursor:
            for date_field in ["created_at", "last_updated"]:
                if date_field in complaint and isinstance(complaint[date_field], datetime):
                    complaint[date_field] = complaint[date_field].isoformat()
...
def update_complaint_status():
    ...
    try:
        ...
        update_result = complaints_collection.update_one(
            {"complaint_id": complaint_id},
            {"$set": {"status": new_status, "admin_notes": admin_notes, "last_updated": datetime.utcnow()}},
        )
    ...
    try:
        ...
        if complaint_id and complaints_collection is not None:
            complaint = complaints_collection.find_one(
                {"complaint_id": complaint_id},
                {"scenario": 1, "analysis_results": 1, "risk_level": 1, "attack_type": 1, "_id": 0},
            )
            if complaint:
                context = f"""
...
# OTP SYSTEM
# -------------------------------------------------------------------
otp_storage = {}    # phone -> otp  (for women/complaint flow)
otp_verified = {}   # phone -> True
user_otp_storage = {}  # phone -> otp (for citizen login flow)
...
def save_user():
    try:
        ...
        if user_collection is None:
            return jsonify({"success": True, "message": "DB skipped"})

        existing = user_collection.find_one({"phone": phone})
        ...
        return jsonify({"success": True, "message": "Saved skipped"})
...
"""
    ...
    try:
        ...
        if match:
            acts = json.loads(match.group())
        else:
            acts = [{"act": "IPC", "sections": ["375", "376", "376D"], "description": "Rape / Gang Rape"}]

        return acts

    except Exception as e:
        print("[LEGAL AI] Exception:", e)
        return [{"act": "IPC", "sections": ["375", "376", "376D"], "description": "Rape / Gang Rape"}]
...
def submit_complaint_women():
    ...
    if otp_verified.get(phone) != True:
        return jsonify({"status": "error", "message": "OTP not verified yet"}), 400
...
def register_admin():
    ...
    admin_collection.insert_one({"name": data["name"], "email": data["email"], "password": hashed})
...
def member_login():
    ...
    if data["password"] == member["password"]:
        return jsonify({"msg": "Login success", "member_id": str(member["_id"]), "team": member["team"]})
...
def team_performance(team):
    ...
    for member in members:
        output.append({"name": member["name"], "performance": member.get("performance", [])})
    return jsonify(output)


# -------------------------------------------------------------------
# ADMIN VIEWS: WOMEN & CYB COMPLAINTS
# -------------------------------------------------------------------
...
def women_complaints():
    try:
        complaints = list(complaints_collection.find({"complaint_id": {"$regex": "^CMP-"}, "status": "pending"}))
...
def crime_complaints():
    try:
        complaints = list(complaints_collection.find({"complaint_id": {"$regex": "^CYB-"}, "status": "pending"}))
        complaints = [serialize_doc(c) for c in complaints]
        return jsonify(complaints)
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def assign_complaint():
    try:
        ...
        result = complaints_collection.update_one(
            {"complaint_id": complaint_id},
            {"$set": {"assigned_team": team_name, "status": "assigned", "last_updated": datetime.utcnow()}},
        )

        if result.modified_count == 0:
            return jsonify({"error": "Complaint not found or already assigned"}), 404

        return jsonify({"success": True, "message": f"Complaint {complaint_id} assigned to {team_name}"}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_assigned_complaints(team):
    try:
        complaints = list(complaints_collection.find({"assigned_team": team, "status": {"$in": ["assigned", "processing"]}}))
    ...
    try:
        ...
        result = complaints_collection.update_one(
            {"complaint_id": complaint_id, "assigned_team": team},
            {
                ...
                "$push": {"daily_updates": {"date": today_date, "work": today_work}},
            },
        )
...
def complete_complaint():
    try:
        ...
        if report:
            os.makedirs(reports_abs, exist_ok=True)
            report_filename = os.path.join(REPORTS_FOLDER, f"{complaint_id}_{report.filename}")
            full_path = os.path.join(app.root_path, report_filename)
            report.save(full_path)

        result = complaints_collection.update_one(
            {"complaint_id": complaint_id, "assigned_team": team},
            {"$set": {"status": "completed", "completion_report": report_filename, "last_updated": datetime.utcnow()}},
        )
    ...
    try:
        ...
        for c in complaints:
            ...
            teams_dict[team].append({
                ...
                "daily_updates": sorted(c.get("daily_updates", []), key=lambda x: x.get("date")),
...
def team_stats():
    ...
    try:
        pipeline = [
            {"$match": {"assigned_team": {"$exists": True, "$ne": ""}, "status": {"$in": ["assigned", "processing"]}}},
